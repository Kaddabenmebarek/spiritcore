...
@Service
public class SampleListValidator implements Serializable {
	
	private static final long serialVersionUID = -8903946100010014705L;
	@Autowired
	BiotypeService biotypeService;
	@Autowired
	 ...
	 */
	public String[][] validate(File csvFile, BiosampleDto sampleList, User user) throws Exception {
		...
		BiosampleLinker linker = new BiosampleLinker(biotypeService.getMetadataByName(sampleList.getBiotype(), "Checksum"));
		BiosampleQuery biosampleQuery = BiosampleQuery.createQueryForBiotype(sampleList.getBiotype());
		List<BiosampleDto> bios = biosampleService.queryBiosamples(biosampleQuery, user);
		for (BiosampleDto sample : bios) {
			String metadata = biosampleService.getMetadataValue(sample, linker.getBiotypeMetadata());
			if (metadata != null && metadata.equals(checksum))
				throw new Exception("This sample list has already been uploaded.");
		}
		biosampleService.setMetadataValue(sampleList, linker.getBiotypeMetadata(), checksum);
	...
	private ColumnMapper createMetadataMapper(String colName, int position) throws Exception {
		...
		List<BiotypeMetadataDto> metadata = biotypeService.getBiotype("Sample").getMetadatas();
		String metadataName = null;
		for (BiotypeMetadataDto m : metadata) {
	...
	private boolean validateStudyIds(String[][] data) {
		...
		for (int i=1; i<data.length; i++) {
			if (studyService.getStudyByStudyId(data[i][studyIdIdx]) == null) {
	...
	// dose actionGroup
	private String[] doseGroupAcceptedValues = new String[] {"dose actionGroup", "dosegroup"};
